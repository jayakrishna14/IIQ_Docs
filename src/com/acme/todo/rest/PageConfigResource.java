  ...
  extends BasePluginResource
{
  ...
  public PageConfig getPageConfig() throws GeneralException {
    ...
    if (identity == null) {
      throw new GeneralException(Message.localize("TodoPlugin_unable_load_identity"));
    }
    
    Identity.CapabilityManager capabilityManager = identity.getCapabilityManager();
    
    boolean showFlagged = (capabilityManager.hasCapability("SystemAdministrator") || capabilityManager.hasRight("ViewFlaggedUsers"));
  ...
  private boolean isTodoRemovable() {
    return getSettingBool("canDelete");
  }
}